...
const StampTool = ({ stamps, setStamps }: { stamps: StampItem[], setStamps: any }) => {
    ...
    const handleDeleteStamp = (id: string) => {
        if(window.confirm("Bạn có chắc chắn muốn xóa con dấu này khỏi thư viện SignLH?")) {
    ...
    const handleRenameStamp = (id: string, currentName: string) => {
        const newName = prompt("Nhập tên mới cho con dấu:", currentName);
        if (newName && newName.trim()) {
            setStamps((prev: any) => prev.map((s: any) => s.id === id ? { ...s, name: newName.trim() } : s));
        }
    };
...
const ExtractStampTool = ({ setStamps }: { setStamps: any }) => {
                ...
                const response = await ai.models.generateContent({
                    ...
                    contents: {
                        parts: [
                            ...
                            { text: "Restore the stamp in this image. Remove any black text overlaying the stamp. Keep the white background. Do not make the background transparent. Return only the image of the restored stamp." }
                        ]
                    }
                });
    ...
    const saveToLibrary = () => {
        ...
        const name = window.prompt("Nhập tên con dấu để lưu vào thư viện SignLH:", "Stamp_" + Date.now());
        if (name) {
            setStamps((prev: any[]) => [...prev, { id: Date.now().toString(), name: name, url: resultImage }]);
            alert("Đã lưu con dấu vào thư viện SignLH! Bạn có thể dùng nó trong mục 'Đóng Dấu'.");
        }
    };
                             ...
                             <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm animate-in fade-in">
                                 ...
                                 <div className="space-y-4">
                                    ...
                                    <div className="pt-2 flex flex-col gap-2">
                                        ...
                                        >
                                            <Save size={16}/> Lưu vào thư viện SignLH
                                        </button>
                                        <button 
